...
class ImageViewController: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate, CLImageEditorDelegate {
    ...
    //写真を撮影・選択した時に呼ばれるメゾット
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]){
        if info[.originalImage] != nil {
            //撮影・選択された画像を取得する
            let image = info[.originalImage] as! UIImage
            //後でCLOmageEditorライブラリで加工する
    ...
    //CLImageEditorで加工が終わった時に呼ばれるメゾット
    ...
    //CLImageEditorの編集がキャンセルされた時に呼ばれるメゾット
    func imageEditorDidCancel(_ editor: CLImageEditor!){
        //ImageViewController画面を閉じてタブ画面に戻る
